 ...
 *
 * 
 * Configuration should point to an LDAP service or an Squash ad-hoc service if 
 * no LDAP is available.
 *  
 *   
 * Security should ensure that only the current user or an ADMIN authority can actually perform the
 * requested operation, safe for the modification of a password (only the user may access it).
 * 
 * @author bsiri
 *
 */
