 ...
 *  - 'entity-id' : the entity id - 'entity-index' : the position of the entity
 ...
 * default : nothing
 *  - functions.getODataId : what : a function fetching the id from the data
 * param : what $().dataTable().fnGetData() would normally accept default :
 * return fnGetData()["entity-id"]
 *  - enableDnD : if coalesce to true, will enable table drag and drop. If
 * coalesce to false, it will not.
 *  - enableHover : if coalesce to true, will enable lines color change when
 * hovered. If coalesce to false, it will not.
 *  - confirmation popups : if 'confirmPopup' is set, will configure any
 * confirmation dialog used in that table. it's an object
 * 
 * oklabel : label for okay buttons cancellabel : label for cancel buttons
 *  - attachments : If the table finds tds having a given cssClass (see
 * cssMatcher) if will turn them into link to the attachment manager.
 * 'attachments' is an object. It must define at least url. It may also override
 ...
 * decorate some cells with execution statuses. If undefined, nothing will
 * happen. The matched cells are identified by css class 'has-status'.
 * 
 ...
 *  - delete button : if the property 'deleteButtons' is set then will look for
 * cells having the css class 'delete-button'. Configuration as follow : url :
 * the url where to post the 'delete' instruction. Supports placeholders.
 * popupmessage : the message that will be displayed tooltip : the tooltip
...
(function($) {
	...
	squashtm = squashtm || {};
	squashtm.keyEventListener = squashtm.keyEventListener
	...
	function _buggedPicsCallback() {
		...
		$(cells).each(
				function(i, cell) {
					...
					var buggedClass = (data.length > 0) ? "hasBugs"
							: "noBugs";

					var pics = '<span class="' + buggedClass
					+ '"> </span>';
