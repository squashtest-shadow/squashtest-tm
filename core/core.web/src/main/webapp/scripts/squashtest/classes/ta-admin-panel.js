...
function TestAutomationProjectManager(settings){
	...
	var switchTestAutomationOnOff = function(){
		...
		$.post(enableTAURL, {enabled : enabled});
	};
...
/*
	settings : 
		- selector : the jquery selector that will fetch the popup back
		- manager : an instance of the class defined just above
		- listProjectsURL : the url where to send the GET request
		- bindProjectsURL : where to post the new projects to bind
