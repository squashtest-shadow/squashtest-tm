...
************************************************/
	 ...
	 * 
	 * 	control : object having
	 * 		- datepick : the jQuery datepicker
	 * 		- datelabel : the jQuery label displaying the date
	 * 		- checkbx : the jQuery checkbox managing the state of the control
	 * 
	 * 	params : object having
	 * 		- paramName : the string containing the label of the POST parameter
	 * 		- url : the url to post to
	 * 		- initialDate : a string containing the number of milliseconds since the 1st january 1970
	 * 		- isAuto : a boolean telling if the control is in auto state (true), or manual state (false);
 ...
 * 
 * 		formerState : 
 * 			- wasAuto : boolean, telling is the control was automode or not
...
function dpa_cancelEditMode(){
	...
	//no need to reset to manual mode, since entering the edit mode implied to be in manual mode already
...
function dpa_postState(){
	...
	$.ajax({
		...
		success : function(strDate){me.postStateSuccess(strDate, callback);},
		error : function(){me.postStateFailed();},
		dataType : "text",
		url : url				
	});		
