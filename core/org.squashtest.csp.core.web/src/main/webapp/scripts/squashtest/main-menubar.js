...
/*
 * The parameters of that function will be passed as they are to more specific init functions, see code below.
 * Dont forget to add your own parameters in that list, please do not overload the existing one.
 * 
 */
	...
	function initMainMenuBar(objFilter){
		...
		//put the rest of your conf here
	...
	/*
	 * parameter object : 
	 *  - boxSelector : the jquery selector we'll use to find the checkbox back
	 *  - url : the url we need to get/post to
	 *  - linkSelector : the jquery selector we'll use to find the link to the configuration popup
	 *  - enabledTxt : the caption the said link should display if the filter is enabled
	 *  - disabledTxt : the caption the said link should display if the filter is disabled 
	 *  - enabledCallbacks : an array containing callbacks to execute when the filter is enabled
	 */
	...
	function enableProjectFilter(url, bEnabled){
		$.post(url,{isEnabled : bEnabled}, function(){
