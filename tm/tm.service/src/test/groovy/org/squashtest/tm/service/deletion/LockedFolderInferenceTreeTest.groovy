...
class LockedFolderInferenceTreeTest extends Specification {
	...
	then :
		...
		allnodes.collect{it.key} == expectedAllNodeIds
		allnodes.collect {it.depth } == expectedAllNodeDepth
		
		layer0.size == expectedLayer0Size
		layer0.collect{it.key} == expectedLayer0Ids
		layer0.collect{it.depth } == expectedLayer0Depth
		layer0.collect{it.children }.flatten() == layer1
	
		layer1.size == expectedLayer1Size
		layer1.collect{it.key} == expectedLayer1Ids
		layer1.collect{it.depth } == expectedLayer1Depth
		layer1.collect{it.children }.flatten() == layer2
		
		layer2.size == expectedLayer2Size
		layer2.collect{it.key} == expectedLayer2Ids
		layer2.collect {it.depth } == expectedLayer2Depth
		layer2.collect{it.children }.flatten() ==  layer3
		
		layer3.size == expectedLayer3Size
		layer3.collect{it.key} == expectedLayer3Ids
		layer3.collect{it.depth } == expectedLayer3Depth
		layer3.collect{it.children }.flatten() == []
