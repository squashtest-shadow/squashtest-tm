...
@NamedQueries({
	...
	@NamedQuery(name = "CampaignStatistics.testinventorybymilestone", query = "select iter.id as iterid, concat (c.name,' ',iter.name ) as name, itp.executionStatus as status, count(tc) as num "
