...
@NamedQueries({
  ...
  @NamedQuery(name = "milestone.findProjectMilestones", query="select p.milestones from Project p where p.id = :projectId"),
   ...
   //InfoList
	...
	@NamedQuery(name="CustomReportLibraryNode.findConcreteLibrary",query="from CustomReportLibraryNode crln where crln.entityType = 'LIBRARY' and crln.library.project.class ='P' and crln.library.project.id in (:filteredProjectsIds) order by crln.name"),
