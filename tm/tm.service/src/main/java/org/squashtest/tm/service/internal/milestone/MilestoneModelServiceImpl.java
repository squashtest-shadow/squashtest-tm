...
public class MilestoneModelServiceImpl implements MilestoneModelService {
	...
	public Map<Long, JsonMilestone> findAllJsonMilestonesByUser(List<Long> partyIds) {
		Map<Long, JsonMilestone> list = new HashMap<>();
		List<JsonMilestone> iterator = DSL.select(MILESTONE.MILESTONE_ID, MILESTONE.LABEL, MILESTONE.M_RANGE, MILESTONE.STATUS, MILESTONE.END_DATE
			, CORE_USER.LOGIN)
			.from(MILESTONE)
			.join(MILESTONE_BINDING).on(MILESTONE.MILESTONE_ID.eq(MILESTONE_BINDING.MILESTONE_ID))
			.join(ACL_RESPONSIBILITY_SCOPE_ENTRY).on(ACL_RESPONSIBILITY_SCOPE_ENTRY.OBJECT_IDENTITY_ID.eq(MILESTONE_BINDING.PROJECT_ID))
			...
			.join(CORE_USER).on(MILESTONE.USER_ID.eq(CORE_USER.PARTY_ID))
			.where(ACL_CLASS.CLASSNAME.eq("org.squashtest.tm.domain.project.Project"))
		...
		for(JsonMilestone milestone : iterator){
			if (!list.containsKey(milestone.getId())) {
				list.put(milestone.getId(),milestone);
			}
		}
		return list;
	}

	public Map<Long, JsonMilestone> findAllJsonMilestonesByAdmin() {
		Map<Long, JsonMilestone> list = new HashMap<>();
		List<JsonMilestone> iterator = DSL.select(MILESTONE.MILESTONE_ID, MILESTONE.LABEL, MILESTONE.M_RANGE, MILESTONE.STATUS, MILESTONE.END_DATE
			, CORE_USER.LOGIN)
			.from(MILESTONE)
	...
	public List<Long> findAllJsonMilestonesIdsByUser(List<Long> partyIds) {
		List<Long> finalList = new ArrayList<>();
		List<Long> listId = DSL.selectDistinct(MILESTONE_BINDING.MILESTONE_ID)
		.from(MILESTONE_BINDING)
		.join(ACL_RESPONSIBILITY_SCOPE_ENTRY).on(ACL_RESPONSIBILITY_SCOPE_ENTRY.OBJECT_IDENTITY_ID.eq(MILESTONE_BINDING.PROJECT_ID))
			.join(ACL_OBJECT_IDENTITY).on(ACL_OBJECT_IDENTITY.ID.eq(ACL_RESPONSIBILITY_SCOPE_ENTRY.OBJECT_IDENTITY_ID))
			.join(ACL_CLASS).on(ACL_CLASS.ID.eq(ACL_OBJECT_IDENTITY.CLASS_ID))
