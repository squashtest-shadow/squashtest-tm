...
require(
		...
		function(common) {
			require(
					[ "jquery", "app/ws/squashtm.workspace", "domReady",
							"jquery.squash.squashbutton", "jform",
							"squashtest/add-attachment-popup" ],
					function($, WS, domReady) {
						...
						function beginUpload(ticket) {
							$("#attachment-progress-dialog").data("ticket",
									ticket);
							...
							$("#add-attachment-form").ajaxSubmit(
									{
										...
										complete : function(jqXHR) {
											attachmentSubmitComplete(ticket,
													jqXHR);
										},
										target : '#dump'
									});
						...
						function attachmentSubmitComplete(ticket, jqXHR) {
							...
							if (json.maxSize === undefined) {
								...
								var message = AAPS.uploadErrorMessage + " ("
										+ formatToMegabyte(maxSize) + " "
										+ AAPS.megaByteLabel;
						...
						/*******************************************************
						 * ========================================================================================
						 * third section : the final popup giving a summary of
						 * the whole operation.
						 * ============================================================================================
						 ******************************************************/
						...
						function populateAndOpenSummary(json) {
							...
							if (json != null) {
								if (!allTransferSuccessful(json)) {
									...
									$("#attachment-upload-summary").dialog(
											"open");
								} else {
									exitUpload();
								}
						...
						function populateSummary(json) {
							...
							for (i = 0; i < summaries.length; i++) {
								$("#attachment-upload-summary-body")
										.append(
						...
						function pollUploadStatus() {
							var ticket = $("#attachment-progress-dialog").data(
									"ticket");
							url = AAPS.uploadAttachmentUrl + "?upload-ticket="
									+ ticket;
						...
						function uploadUpdateBar(percentage) {
							$("#attachment-progressbar").progressbar("option",
									"value", percentage);
						}

						function uploadDisplayPercentage(percentage) {
							$("#attachment-progress-percentage").html(
									percentage.toString() + " &#37;");
						}
						...
						domReady(function() {
							...
							var formInstance = $("#add-attachment-form")
									.uploadPopup(itemTemplate);
							$("#attachment-upload-dialog").data("formInstance",
									formInstance);

							/**
							 * more initiatisation code for the progress bar
							 * popup *
							 */
							...
							$("#attachment-progress-dialog").bind(
