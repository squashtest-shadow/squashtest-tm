...
define(
		...
		function($, Forms) {
			...
			function handleJsonResponseError(request) {
				/*
				 * this pukes an exception if not valid json. there's no other
				 * jQuery way to tell
				 */
			...
			function showLegacyErrorMessage(fieldValidationError) {
				...
				labelId = labelId.replace(".", "-").replace('[', '-').replace(
						']', '');// this is necessary because labelId is used
									// as a css classname
			...
			function handleGenericResponseError(request) {
				...
				$('#show-generic-error-details').unbind('click').click(
						showError);

				$('#generic-error-notification-area').fadeIn('slow').delay(
						20000).fadeOut('slow');
			}
