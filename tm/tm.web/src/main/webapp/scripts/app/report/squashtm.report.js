		...
		function($, RWS) {
			...
			function onDatepickerChanged(value) {
				...
				if (config.noDateLabel === value) {
					...
					var date = $.datepicker.parseDate(config.dateFormat,
							localizedDate);
			...
			function onViewTabSelected(event, ui) {
				...
				tabs.find("#view-format-cmb-" + ui.newTab.index()).removeClass(
						'not-displayed');
			...
			/**
			 * Fetches the workspace type from the given jquery object pointing
			 * to a treepicker
			 */
