		...
		function($, TableCollapser, cufValuesManager) {
			...
			function makeTableUrls(conf) {
				...
				return {
					...
					attachments : ctxUrl
							+ "/attach-list/{attach-list-id}/attachments/manager?workspace=test-case",
					...
					stepcufBindingUrl : ctxUrl
							+ "/custom-fields-binding?projectId="
							+ conf.basic.projectId
			...
			function stepsTableCreatedRowCallback(nRow, aData, iDisplayIndex,
					iDisplayIndexFull) {
				nRow.className += (aData['step-type'] === "action") ? " action-step-row"
						: " call-step-row";
			}
			...
			function specializeCellClasses(table) {
				...
				actionRows.find('td.called-tc-cell').removeClass(
						'called-tc-cell');

				// remove useless classes for call step rows
				callRows.find('td.rich-edit-action').removeClass(
						'rich-edit-action');
				callRows.find('td.rich-edit-result').removeClass(
						'rich-edit-result');
				callRows.find('td.has-attachment-cell').removeClass(
						'has-attachment-cell');
				callRows.find('td.custom-field-value').removeClass(); // remove
																		// all
																		// the
																		// classes
				callRows.find('td.called-tc-cell').next().remove().end().attr(
						'colspan', 2);
			}
			...
			function initTable(settings) {
				...
				cufTableHandler.decorateDOMTable($("#test-steps-table"),
						settings.basic.cufDefinitions, cufColumnPosition);
				...
				var datatableSettings = {
					...
					aoColumnDefs : [
							...
							},
							{
								...
								'sClass' : 'called-tc-cell collapsible '
										+ editActionClass
							}, {
								'bVisible' : true,
								'bSortable' : false,
				...
				datatableSettings = cufTableHandler.decorateTableSettings(
						datatableSettings, settings.basic.cufDefinitions,
					...
					buttons : [
							...
							},
							{
								...
								onClick : function(table, cell) {
									...
									var url = urls.steps
											+ stepId
											+ "/verified-requirement-versions/manager";
			...
			function readAddStepParams() {
				...
				var cufSupport = $("#add-test-step-dialog").data(
						'cuf-values-support');
			...
			function initAddTestStepDialog(language, urls) {
				...
				decorateStepTableButton("#add-test-step-button",
						"ui-icon-plusthick");
				...
				var cufValuesSupport = cufValuesManager
						.newCreationPopupCUFHandler({
							url : bindingsUrl,
							table : cufTable
						});
			...
			function initCallStepButton(urls) {
				...
				decorateStepTableButton("#add-call-step-button",
			...
			function makeCollapsibleCellsHandlers(collapser) {
				...
				return {
					open : function(collapser) {
						var collapsibleCells = $(collapser.collapsibleCells);
						collapsibleCells.addClass('editable').off("click",
								openEdit).editable('enable');
					},

					close : function(collapser) {
						var collapsibleCells = $(collapser.collapsibleCells);
						collapsibleCells.removeClass('editable').on("click",
								openEdit).editable('disable');
					}
				};
			}
			...
			function initCollapser(language, urls, isWritable) {
				...
				decorateStepTableButton("#collapse-steps-button",
						"ui-icon-zoomout");
				...
				collapseButton.click(function() {
					...
					if (collapser.isOpen) {
						if (isEditing(collapser)) {
							$.squash.openMessage(language.infoTitle,
									language.collapseMessage);
						} else {
							collapser.closeAll();
						}
					} else {
