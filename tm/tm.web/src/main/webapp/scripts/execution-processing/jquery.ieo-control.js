...
define(
		[ "jquery", "module", "jquery.cookie", "jqueryui" ],
		function($, module) {
					...
					.widget(
							"squash.ieoControl",
							{
								...
								_create : function() {
									...
									var positionLeft = $
											.cookie("ieo-toolbox-position-left");
									var positionTop = $
											.cookie("ieo-toolbox-position-top");
									...
									this.element
											.draggable({
												stop : function(event, ui) {
													...
													$
															.cookie(
																	"ieo-toolbox-position-left",
																	pos.left);
													$
															.cookie(
																	"ieo-toolbox-position-top",
																	pos.top);
												}
											});
								...
								getNextStepButton : function() {
									return this.element
											.find('.execute-next-step');
								},

								getPreviousStepButton : function() {
									return this.element
											.find('.execute-previous-step');
								},
								...
								getNextTestCaseButton : function() {
									return this.element
											.find('.execute-next-test-case');
								},

								getStatusCombo : function() {
									return this.element
											.find('.execution-status-combo-class');
								},
								...
								_canNavigateNextTestCase : function() {
									...
									return ((state.testSuiteMode)
											&& (!state.lastTestCase) && (this
											._isLastStep()));
								},
								...
								_initSlider : function() {
									...
									var settings = (!!this.element.manager) ? this
											._getState()
											: {
												lastStepIndex : 0,
												currentStepIndex : 0
									...
									var sliderSettings = {
										...
										stop : function(event, ui) {
											self.element.manager
													.navigateRandom(ui.value);
										}
									};
								...
								_updateComboIcon : function() {
									...
									cbox
											.addClass("execution-status-combo-class");
									...
									var className = cbox.find(selector).attr(
											"class");
								...
								_updateCounter : function() {
									var label = this.element
											.find('.step-paging');
									var state = this._getState();
									var labelText = state.currentStepIndex
											+ " / " + state.lastStepIndex;
								...
								_updateButtons : function() {
									...
									var btnState = (this._isLastStep()) ? "disable"
											: "enable";
									this.getNextStepButton().button(btnState);

									btnState = (this._isPrologue()) ? "disable"
											: "enable";
									this.getPreviousStepButton().button(
											btnState);
									...
									btnState = (this._canNavigateNextTestCase()) ? "enable"
											: "disable";
									this.getNextTestCaseButton().button(
											btnState);
								...
								_updateSlider : function() {
									this.getSlider().slider("option", "value",
											this._getState().currentStepIndex);
								},

								_updateComboEnable : function() {
									if (this._isPrologue()) {
										this.getStatusCombo().prop('disabled',
												true);
									} else {
										this.getStatusCombo().prop('disabled',
												false);
