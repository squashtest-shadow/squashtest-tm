...
(function($) {
					...
					"squash",
					{
						...
						_fn : {
							...
							selectionIsCopyable : function(selectedNodes) {
								...
								var isCreate = this
										.selectionIsCreatable(selectedNodes);
								if (isCreate != "OK")
									return isCreate;
								else if (selectedNodes.is(":library"))
							...
							container
									.bind(
											...
											function(event, data) {
												...
												unselectDescendantsAndOtherProjectsSelections(
														data.rslt.obj, self);
	 ...
	 * were selected and redefine get_selected to return the nodes in that
	 * order.
	 */
	 ...
	 * 
	 * @param contentSelector : the selector of the content.
	 * @param params :
	 *            a map association <buttonPropertyName, buttonSelector>.
	 * @param width : optional menu width, defaults to 180
	 * 
	 * Alternatively, can be called with a hash unique param : 
	 * @param options : {html : "html content of the menu", params : {}, width: width}
	 * 
	 * Note 1 : the way the menu was implemented forces us to ugly things and
	 * should need refactoring once it's included in the trunk of jQuery UI.
	 * Note 2 : I had no choice but modifying jquery.fg.menu.js directly,
