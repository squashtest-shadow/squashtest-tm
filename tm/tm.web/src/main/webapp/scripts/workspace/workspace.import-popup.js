...
define(["jquery", "underscore", "app/ws/squashtm.notification", "squash.translator", "jquery.squash.formdialog", "jform"], function ($, _, WTF, translator) {
	...
	$.widget("squash.importDialog", $.squash.formDialog, {
		...
		bindEvents: function () {
			...
			this.onOwnBtn("statechangeparametrization", function () {
				self.changeTitle(translator.get("dialog.import-excel.title"));
			});
			this.onOwnBtn("statechangesummary", function () {
				self.changeTitle(translator.get("dialog.import-excel.title.summary"));
			});
