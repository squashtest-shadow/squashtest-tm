...
define(
		...
		function($, Backbone, _, StringUtil) {
			...
			var VerifiedRequirementsTable = Backbone.View
					.extend({
						...
						initialize : function() {
							...
							this.removeRequirements = $.proxy(
									this._removeRequirements, this);
							this.removeRowRequirementVersion = $.proxy(
									this._removeRowRequirementVersion, this);
							this.requirementsTableDrawCallback = $.proxy(
									this._requirementsTableDrawCallback, this);
							this.requirementsTableRowCallback = $.proxy(
									this._requirementsTableRowCallback, this);
							this.removeSelectedRequirements = $.proxy(
									this._removeSelectedRequirements, this);
							this.confirmRemoveRequirements = $.proxy(
									this._confirmRemoveRequirements, this);
							this.addSelectEditableToVersionNumber = $.proxy(
									this._addSelectEditableToVersionNumber,
									this);
						...
						_requirementsTableDrawCallback : function() {
							if (this.table) {// We do not restore table
								...
								restoreTableSelection(this.table,
										function(data) {
						...
						_confirmRemoveRequirements : function(rows) {
							...
							if (hasRequirement) {
								...
								var obsoleteStatuses = $(rows)
										.not(
												function(index, row) {
													return self.table
															.fnGetData(row)["status"] != "OBSOLETE";
												});
								if (obsoleteStatuses.length > 0) {
									this.confirmRemoveObsoleteRequirementDialog
											.confirmDialog("open");
								} else {
									this.confirmRemoveRequirementDialog
											.confirmDialog("open");
								}
							} else {
						...
						configureRemoveRequirementDialogs : function() {
							...
							this.confirmRemoveRequirementDialog = $(
									"#remove-verified-requirement-version-dialog")
									.confirmDialog();
							this.confirmRemoveRequirementDialog.width("600px");
							this.confirmRemoveRequirementDialog.on(
									"confirmdialogconfirm", $.proxy(
											this.removeRequirements, this));
							this.confirmRemoveRequirementDialog.on("close", $
									.proxy(function() {
										this.toDeleteIds = [];
									}, this));
							// confirmRemoveObsoleteRequirementDialog
							this.confirmRemoveObsoleteRequirementDialog = $(
									"#remove-obsolete-verified-requirement-version-dialog")
									.confirmDialog();
							this.confirmRemoveObsoleteRequirementDialog
									.width("600px");
							this.confirmRemoveObsoleteRequirementDialog.on(
									"confirmdialogconfirm", $.proxy(
											this.removeRequirements, this));
							this.confirmRemoveObsoleteRequirementDialog.on(
						...
						configureNoRequirementSelectedDialog : function() {
							this.noRequirementSelectedDialog = $(
									"#no-selected-requirement-dialog")
									.messageDialog();
						},
