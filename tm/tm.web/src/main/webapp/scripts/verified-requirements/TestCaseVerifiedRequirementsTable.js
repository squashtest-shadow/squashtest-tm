...
define(
		...
		function($, Backbone, _, StringUtil, VerifiedRequirementsTable) {
			...
			var TestCaseVerifiedRequirementsTable = VerifiedRequirementsTable
					.extend({

						initialize : function(options) {
							this.constructor.__super__.initialize.apply(this,
									[ options ]);
							this.configureNoDirectRequirementSelectedDialog
									.call(this);
						},
							...
							return {
								buttons : [ {
									...
									condition : function(row, data) {
										var verified = data["directlyVerified"] == "false" ? false
												: data["directlyVerified"];
								...
								var indirects = $(rows)
										.not(
												function(index, row) {
													var data = self.table
															.fnGetData(row);
													return data["directlyVerified"] == "false" ? false
															: data["directlyVerified"];
												});
								if (indirects.length > 0) {
									this.noDirectRequirementSelectedDialog
											.messageDialog("open");
								} else {
						...
						_requirementsTableRowCallback : function(row, data,
							...
							var verified = data["directlyVerified"] == "false" ? false
									: data["directlyVerified"];
							if (VRTS.linkable && verified
									&& data["status"] != "OBSOLETE") {
								this
										.addSelectEditableToVersionNumber(row,
												data);
							}
							this.discriminateDirectVerifications(row, data,
									displayIndex);
						...
						configureNoDirectRequirementSelectedDialog : function() {
							this.noDirectRequirementSelectedDialog = $(
									"#no-selected-direct-requirement-dialog")
									.messageDialog();
						},

						// =====================================================

						discriminateDirectVerifications : function(row, data,
								displayIndex) {
							var verified = data["directlyVerified"] == "false" ? false
									: data["directlyVerified"];
							if (!verified) {
								$(row).addClass(
										"requirement-indirect-verification");
								$('td.delete-button', row).html(''); // remove
																		// the
																		// delete
																		// button
							} else {
