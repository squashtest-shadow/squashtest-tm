	...
	var TestStepVerifiedRequirementsTable = VerifiedRequirementsTable.extend({
		initialize : function(options) {
			...
			var checkboxTemplate =  
				'{{#if checked}}<input type="checkbox" id="{{id}}" data-version-id="{{id}}" name="verified-by-step-checkbox" checked="checked" class="not-displayed" />' +
			...
			this.checkbox = Handlebars.compile(checkboxTemplate);
		},
		...
		addLinkCheckboxToRow : function(row, data, displayIndex) {
			...
			var linkIconClass = function(checked) {
				return checked ? "ui-icon-link-dark-e-w" : "ui-icon-link-clear-e-w";
			};
			...
			var onChangeCheckbox = function() {
				...
				$label.toggleClass("ui-icon-link-dark-e-w").toggleClass("ui-icon-link-clear-e-w");
			};

			if (VRTS.linkable) {
				$checkbox.on("change", sendLinkedToStep)
