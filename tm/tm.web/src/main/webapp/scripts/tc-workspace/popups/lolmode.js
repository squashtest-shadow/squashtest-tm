...
define(["jquery", 'workspace/workspace.delnode-popup', "jqueryui"], 
	...
	var WOBBLE_SPAN_UP = {
		...
		'margin-bottom' : DEFAULT_SPAN_STATE['margin-bottom'] + WOBBLE_MAX
	};
...
/*
	...
	function LolMode(dialog){
		...
		this.runspan.text(RIGHT_SPAN_RUN);
		...
		this.clean = function(){
			dialog.off('delnodedialogdeadlyconfirm ');
		...
		this.die = function(){
			...
			var newtxt = runspan.text().replace(/\^/g,'°');
			runspan.text(newtxt);
