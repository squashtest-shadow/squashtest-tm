...
define(['jquery', 'tree', '../permissions-rules', 'jquery.squash.formdialog'], function($, zetree, rules){
	...
	function postNode(dialog, tree){
		...
		var params = {
			name : dialog.find('#add-global-dataset-name').val(),
			reference : dialog.find('#add-global-dataset-reference').val(),
			description : dialog.find('#add-global-dataset-description').val()
		};

		return tree.jstree('postNewNode', 'new-global-dataset', params, false);
	}


	function init(){

		var dialog = $("#add-global-dataset-dialog").formDialog();
		var tree = zetree.get();
		...
		dialog.on('formdialogopen', function(){
			...
			if (! rules.canCreateGlobalDataset(node)){
...
//				dialog.formDialog('setState', errorState);
				...
				dialog.find("#new-global-dataset-tree-button").val(name);
			}
		});
