...
define(['jquery', 'tree', '../permissions-rules', 'jquery.squash.formdialog'], function($, zetree, rules){
	...
	function postNode(dialog, tree){
		...
		var params = {
			name : dialog.find('#add-dataset-template-name').val(),
			reference : dialog.find('#add-dataset-template-reference').val(),
			description : dialog.find('#add-dataset-template-description').val()
		};

		return tree.jstree('postNewNode', 'new-dataset-template', params, false);
	}


	function init(){

		var dialog = $("#add-dataset-template-dialog").formDialog();
		var tree = zetree.get();
		...
		dialog.on('formdialogopen', function(){
			...
			if (! rules.canCreateDatasetTemplate(node)){
...
//				dialog.formDialog('setState', errorState);
				...
				dialog.find("#new-dataset-template-tree-button").val(name);
			}
		});
