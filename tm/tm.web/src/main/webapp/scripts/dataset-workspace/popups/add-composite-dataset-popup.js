...
define(['jquery', 'tree', '../permissions-rules', 'jquery.squash.formdialog'], function($, zetree, rules){
	...
	function postNode(dialog, tree){
		...
		var params = {
			name : dialog.find('#add-composite-dataset-name').val(),
			reference : dialog.find('#add-composite-dataset-reference').val(),
			description : dialog.find('#add-composite-dataset-description').val()
		};

		return tree.jstree('postNewNode', 'new-composite-dataset', params, false);
	}


	function init(){

		var dialog = $("#add-composite-dataset-dialog").formDialog();
		var tree = zetree.get();
		...
		dialog.on('formdialogopen', function(){
			...
			if (! rules.canCreateCompositeDataset(node)){
...
//				dialog.formDialog('setState', errorState);
				...
				dialog.find("#new-composite-dataset-tree-button").val(name);
			}
		});
