...
define(
		...
		function($) {
			...
			/*
			 * this version of fg-menu is able to replace the content
			 * dynamically, recreating the internal structure when needed.
			 */
			...
			function TestSuiteMenu(settings) {
				...
				var initializeContent = $.proxy(function() {
					...
					var okCancellButtons = this.okCancellButtons
							.getButtonsHtml();
					// now set the content
				...
				var getCheckboxes = $.proxy(function() {
					return this.menu.getContainer().find(
							'input[name="menu-suite-item"]');
				}, this);

				/*
				 * if the menu was open by the time we (re)defined the content,
				 * we must create it now (and hope that the refresh will be fast
				 * enough to the eyes of the user) else, we simply reset the
				 * 'menuExists' flag, and the popup will redraw itself next time
				 * we open it.
				 */
				...
				var displayAddSuiteError = $
						.proxy(
								function(xhr, text) {
									try {
										var errContent = jQuery
										...
										if (errContent.fieldValidationErrors !== undefined) {
											...
											message
													.append('<div>could not add your suite : unexpected error</div>');
										}
				...
				var addSuiteToMenuContent = $.proxy(function(evt) {
					...
					this.menu.getContainer().find(
							'ul.suite-manager-menu-mainlist').append(item);
				}, this);

				var addSuite = $.proxy(function() {
					var self = this;
					var name = this.menu.getContainer().find(
							'.suite-manager-menu-input').val();
				...
				var stopEventPropagation = $.proxy(function() {
					...
					container.delegate('div, ul, li,  label', 'click',
							function(evt) {
					...
					container.delegate('input:checkbox', 'change',
							function(evt) {
								...
								if (checkbx.is(":checked")) {
									...
									self.checkedSuites.remove(checkbx
											.data('suite-id'));
								}
											...
											.delegate(
													...
													function(evt) {
														...
														if (!getDatatableSelected().length) {
															...
															var toSend = {};
															var suiteIds = self.checkedSuites
																	.get();
															if (suiteIds.length < 1) {
																$(
																		settings.emptySuiteSelectionMessageSelector)
																		.openMessage();
															} else {
				...
				var bindCancelButton = $.proxy(function() {
					var container = this.menu.getContainer();
					var self = this;
					container.delegate('.suite-manager-menu-cancel-button',
							'click', function(evt) {
								evt.stopImmediatePropagation();
								self.menu.kill();
							});
				...
				var bindAddButton = $.proxy(function() {
					var container = this.menu.getContainer();
					container.delegate('.suite-manager-menu-button', 'click',
							function(evt) {
								evt.stopImmediatePropagation();
								addSuite();
							});
				...
				var bindInput = $.proxy(function() {
					var container = this.menu.getContainer();
					container.delegate('.suite-manager-menu-input', 'click',
							function(evt) {
								evt.stopImmediatePropagation();
							});
					container.delegate('.suite-manager-menu-input', 'keypress',
							function(evt) {
								evt.stopImmediatePropagation();
								if (evt.which == '13') {
									addSuite();
