...
define(
		...
		function($) {
			...
			function TestSuiteManagerControl(settings) {
				...
				this.button.click(function() {
					if (self.confirmMessage && self.confirmTitle) {
						oneShotConfirm(self.confirmTitle, self.confirmMessage,
			...
			/*
			 * 
			 * The view displays and manage the test suites (known here as
			 * items)
			 * 
			 */
			function TestSuiteManagerView(settings) {
				...
				this.getSelectedIds = function() {
					var ids = this.getSelected().find('span').collect(
							function(elt) {
				...
				this.update = function(evt) {
					...
					if ((evt === undefined) || (evt.evt_name == "add")
							|| (evt.evt_name == "rename")
							|| (evt.evt_name == "remove")
			...
			function TestSuiteManager(settings) {
				...
				this.remove = {};
				this.remove.button = $(".remove-suites-section input",
						this.instance);
				...
				var renameControlSettings = {
					manager : this,
					defaultMessage : '',
					panel : this.rename.panel = $(".rename-suites-section",
							this.instance),
				 ...
				 * has no text input, just a button
				 */
				var removeControlSettings = {
					...
					defaultMessage : settings.defaultMessage,
					panel : this.remove.panel = $(".remove-suites-section",
							this.instance),
				...
				this.rename.control = new TestSuiteManagerControl(
						renameControlSettings);
				this.remove.control = new TestSuiteManagerControl(
