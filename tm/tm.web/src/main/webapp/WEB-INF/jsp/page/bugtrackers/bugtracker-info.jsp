...
<layout:info-page-layout titleKey="workspace.bugtracker.info.title" isSubPaged="true">
  ...
  <jsp:attribute name="subPageButtons">
		...
		<input type="button" class="button" value="${backButtonLabel}"
           onClick="document.location.href='${admBugtrackerUrl}'"/>
	</jsp:attribute>
  ...
  <jsp:attribute name="informationContent">
      ...
      <comp:toggle-panel id="bugtracker-info-panel"
            ...
            <div class="display-table-row">
              ...
              <comp:select-jeditable componentId="bugtracker-kind" jsonData="${bugtrackerKinds}"
                                     targetUrl="${bugtrackerUrl}"/>
            </div>
            ...
            <div class="display-table-row">
              ...
              <div class="display-table-cell editable text-editable" data-def="url=${bugtrackerUrl}"
                   id="bugtracker-url">${ bugtracker.url }</div>
            </div>
        ...
        <comp:toggle-panel id="bugtracker-auth-protocol" titleKey="label.BugtrackerAuthProtocol" open="true">
            ...
            <div class="srv-form srv-auth-form-main side-panel std-border std-border-radius"
              ...
              <div class="centered srv-auth-buttonpane" style="position:relative">
                <span class="needs-save-msg" style="display:none;"><f:message
                  key="bugtracker.admin.messages.needs-save"/></span>
          ...
          <div class="tbl" style="width:100%">
            ...
            <div>
              ...
              <div class="flexible">
                <select data-bind="requestTokenHttpMethod">
                  <option value="GET" {{#equal requestTokenHttpMethod
                  'GET'}}selected="selected"{{/equal}}>GET</option>
                  <option value="POST" {{#equal requestTokenHttpMethod
                  'POST'}}selected="selected"{{/equal}}>POST</option>
                </select>
            ...
            <div>
              ...
              <div class="flexible">
                <select data-bind="accessTokenHttpMethod">
                  <option value="GET" {{#equal accessTokenHttpMethod
                  'GET'}}selected="selected"{{/equal}}>GET</option>
                  <option value="POST" {{#equal accessTokenHttpMethod
                  'POST'}}selected="selected"{{/equal}}>POST</option>
                </select>
            ...
            <div>
              ...
              <select data-bind="signatureMethod">
                <option value="HMAC_SHA1" {{#equal signatureMethod
                'HMAC_SHA1'}}selected="selected"{{/equal}}>HMAC-SHA1</option>
                <option value="RSA_SHA1" {{#equal signatureMethod
                'RSA_SHA1'}}selected="selected"{{/equal}}>RSA-SHA1</option>
              </select>
            </div>
          </div>
        </script>
