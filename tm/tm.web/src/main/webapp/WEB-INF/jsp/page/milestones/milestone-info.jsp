...
<layout:info-page-layout titleKey="workspace.milestone.info.title" isSubPaged="true">
  ...
  <f:message var="delete" key='button.delete.label'/>
         <input type="button" value="${ delete }" id="delete-milestone-button" class="sq-btn"
                title="<f:message key='milestone.button.delete.label' />"/>
              </c:if>
        </div>
      </div>
        <%--------End Toolbar ---------------%>
        ...
        <jsp:attribute name="body">
          <div id="milestone-description-table" class="display-table">
            ...
            <div class="display-table-row">
              ...
              <c:choose>
                  <c:when test="${ canEdit }">
              <div class="display-table-cell editable rich-editable" data-def="url=${milestoneUrl}"
                   id="milestone-description">${ hu:clean(milestone["description"]) }</div>
                 </c:when>
             <c:otherwise>
    ...
    <jsp:attribute name="panelButtons">
      <c:if test="${canEdit }">
        <button id="bind-project-button" title="<f:message key="label.milestone.bindProject" />"
        ...
        <table id="projects-table" class="unstyled-table"
               data-def="ajaxsource=${projectsUrl}?binded, hover, filter, pre-sort=2-asc"
          ...
          <tr>
            ...
            <th data-def="map=binded, sortable, sClass=binded-to-project" class="datatable-filterable"><f:message
              key="label.project.isBoundToMilestone"/></th>
...
<div id="bind-project-dialog" class="not-displayed popup-dialog"
  ...
  <table id="bind-to-projects-table" class="unstyled-table"
         data-def="ajaxsource=${projectsUrl}?bindable, hover, filter, pre-sort=1-asc">
    <thead>
    <tr>
