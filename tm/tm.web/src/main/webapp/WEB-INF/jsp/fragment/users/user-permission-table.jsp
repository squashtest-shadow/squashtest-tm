...
<%@ page language="java" contentType="text/html; charset=utf-8" pageEncoding="utf-8"%>
	...
	<table id="project-permission-table">
		<thead>
			<tr>
				<th> Id </th>
				<th> <f:message key="projects.label" /></th>
		...
		<tbody>
			...
			<c:forEach items="${projectPermissionList}" var="permissionProject">
				<tr id="${permissionProject.project.id}">
					<td id="id">${permissionProject.project.id}</td>
					<td>
						<span title="${permissionProject.project.label}">${permissionProject.project.name}</span>
					</td>
					<td>
						<select class="select-class">
							<c:forEach items="${permissionList}" var="permission">
								<option value="${permission.qualifiedName}"
									...
									<c:if test="${permissionProject.permissionGroup.simpleName == permission.simpleName}">
										selected="selected"
									</c:if>
									>
									<f:message
