...
<div id="test-suite-toolbar" class="toolbar-class ui-corner-all cf">
  ...
  <div class="toolbar-button-panel btn-toolbar right">
    ...
    <c:if test="${ writable }">
      <input type="button" value="<f:message key='test-suite.button.rename.label' />" id="rename-test-suite-button" title="<f:message key='dialog.testsuites.rename.title' />"
        class="sq-btn" title="<f:message key='test-suite.button.rename.label' />"/>
    </c:if>
    <c:if test="${ creatable }">
      <input type="button" value="<f:message key='test-suite.button.duplicate.label' />" title="<f:message key='title.DuplicateTestSuite' />"
...
<c:if test="${testSuite.project.bugtrackerConnected}">
    ...
    <ts:test-suite-test-plan-panel assignableUsers="${assignableUsers}" testSuite="${testSuite}" weights="${weights}"
      modes="${modes}" statuses="${statuses}" editable="${writable}" executable="${executable}" linkable="${linkable}"
