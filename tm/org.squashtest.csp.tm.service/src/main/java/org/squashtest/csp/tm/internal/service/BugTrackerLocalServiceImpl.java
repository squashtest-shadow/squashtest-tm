...
public class BugTrackerLocalServiceImpl implements BugTrackerLocalService{
	...
	@Override
	@PreAuthorize("hasPermission(#entity, 'WRITE') or hasRole('ROLE_ADMIN')")
	...
	public FilteredCollectionHolder<List<IssueOwnership<Issue>>> findSquashIssues(Bugged buggedEntity,
		...
		FilteredCollectionHolder<List<IssueOwnership<Issue>>> result = 
			new FilteredCollectionHolder<List<IssueOwnership<Issue>>>(totalIssues, pairedIssues);
	 ...
	 * 		
	 * 		1) get the sorted/filtered yet unpaired squash issues first,
	 * 		2) get the corresponding remote issues,
	 * 		3) get their owner in the squash DB again by matching the remoteIssueId
	 * 
	 ...
	 * 
	 * (non-Javadoc)
	 * @see org.squashtest.csp.tm.service.BugTrackerLocalService#findBugTrackerIssues(org.squashtest.csp.tm.domain.bugtracker.Bugged, org.squashtest.csp.tm.infrastructure.filter.CollectionSorting)
	 */
	
	@Override
	@PreAuthorize("hasPermission(#buggedEntity, 'READ') or hasRole('ROLE_ADMIN')")
	public FilteredCollectionHolder<List<IssueOwnership<BTIssue>>> findBugTrackerIssues(
			Bugged buggedEntity, CollectionSorting sorter) {

		FilteredCollectionHolder<List<IssueOwnership<Issue>>> filteredIssues = 
			findSquashIssues(buggedEntity, sorter);
		...
		List<IssueOwnership<BTIssue>> btOwnership = makeBTIssueOwnership(remoteIssues, filteredIssues.getFilteredCollection());
		
		FilteredCollectionHolder<List<IssueOwnership<BTIssue>>> toReturn = 
			new FilteredCollectionHolder<List<IssueOwnership<BTIssue>>>(filteredIssues.getUnfilteredResultCount(), btOwnership);
	...
	private List<IssueOwnership<BTIssue>> makeBTIssueOwnership(List<BTIssue> remoteIssues, List<IssueOwnership<Issue>> localIssues ){
		...
		for (BTIssue remoteIssue : remoteIssues){
			...
			while(iterator.hasNext()){
				...
				if (localOwnership.getIssue().getRemoteIssueId().equals(remoteIssue.getId())){
					IssueOwnership<BTIssue> remoteOwnership = new IssueOwnership<BTIssue>(remoteIssue, localOwnership.getOwner());
