...
public class CampaignDeletionHandlerImpl extends
		AbstractNodeDeletionHandlerImpl<CampaignLibraryNode, CampaignFolder> implements
	...
	@Override
	public List<SuppressionPreviewReport> simulateExecutionDeletion(
			Long execId) {
	 ...
	 *  
	 * TODO : 
	 * 		- implement a careful deletion procedure once the policies and rules are defined.
	...
	protected void batchDeleteNodes(List<Long> ids) {
		 ...
		 * while the rest will be executed using SQL queries. The inconsistencies between cached entities but not yet flushed 
		 * entities and the actual database content would make the operation crash, so we need to synchronize. 
		 */
		deletionDao.flush();			
	...
	/*
	 * we just remove the content of a campaign here. The actual
	 * removal of the campaign will be processed in the calling methods.
	 * 
	 * The operations carried over a campaign are :
	 * - removal of all its iterations,
	 * - removal its attachment list,
	 * 
