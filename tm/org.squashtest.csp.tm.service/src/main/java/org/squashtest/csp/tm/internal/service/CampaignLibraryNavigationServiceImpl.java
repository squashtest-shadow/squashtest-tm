...
public class CampaignLibraryNavigationServiceImpl
		extends
		AbstractLibraryNavigationService<CampaignLibrary, CampaignFolder, CampaignLibraryNode>
	...
	public int addIterationToCampaign(Iteration iteration, long campaignId) {
		...
		if (!campaign.isContentNameAvailable(iteration.getName())) {
			throw new DuplicateNameException(iteration.getName(),
					iteration.getName());
		}
		return iterationModificationService.addIterationToCampaign(iteration,
				campaignId);
	}
	...
	public List<Iteration> findIterationsByCampaignId(long campaignId) {
		return iterationModificationService
				.findIterationsByCampaignId(campaignId);
	}

	/*
	 * //TODO : investigate why that method is public and exposed in the
	 * interface.
	 * 
	 * (non-Javadoc)
	 * 
	 * @see org.squashtest.csp.tm.service.CampaignLibraryNavigationService#
	 * createCopyCampaign(long)
	 */
	@Override
	@PreAuthorize("hasPermission(#campaignId,'org.squashtest.csp.tm.domain.campaign.Campaign','READ') or hasRole('ROLE_ADMIN')")
	public Campaign createCopyCampaign(long campaignId) {
		Campaign original = campaignDao
				.findByIdWithInitializedIterations(campaignId);
		Campaign clone = original.createCopy();

		/*
		 * The mapping imposes us to persist the iterations first, since
		 * persisting the campaign only won't cascade properly and generate an
		 * exception. See the comment in Iteration.campaign.
		 * 
	...
	/*
	 * //TODO : if the TODO for createCopyCampaign just above concludes that it
	 * really has a purpose, then find why createCopyIteration is not exposed in
	 * the interface too.
	 */
	...
	public void addCampaignToCampaignLibrary(long libraryId,
		...
		if (!library.isContentNameAvailable(newCampaign.getName())) {
			throw new DuplicateNameException(newCampaign.getName(),
					newCampaign.getName());
		} else {
	...
	/*
	 * //TODO : investigate why that method must always return 0, or why should
	 * it return something anyway.
	 * 
	 * (non-Javadoc)
	 * 
	 * @see org.squashtest.csp.tm.service.CampaignLibraryNavigationService#
	 * moveIterationToNewCampaign(long, long, long)
	 */
	@Override
	...
	public int moveIterationToNewCampaign(long newCampaignId,
			long oldCampaignId, long iterationId) {
	...
	public List<CampaignLibrary> findLinkableCampaignLibraries() {
		ProjectFilter pf = projectFilterModificationService
				.findProjectFilterByUserLogin();
		return pf.getActivated() == true ? libraryStrategy
				.getSpecificLibraries(pf.getProjects()) : campaignLibraryDao
				.findAll();
	}

	@Override
	public List<SuppressionPreviewReport> simulateIterationDeletion(
			List<Long> targetIds) {
