...
/**
 * The following specialization for NodeDeletionHandler, meant for the campaigns, will also handle the deletion
 * of campaign related entities like Iterations and Executions. Thus the contract of the interface NodeDeletionHandler
 * is extended for those entities.
 * 
 * First, some definitions :
 * 
 * <ul>
 * 		<li> Targeted deletion : the user designated one or several entities to be specifically deleted </li>
 * 		<li> Cascaded deletion : the entities that become deleted due to the cascade mechanisms (initiated by a 
 * 			targeted deletion higher in the hierarchy</li>
 * </ul>
 * 
 * 
 * <b>In the context of a targeted deletion</b>, Iterations and Executions may require a preview of the deletion operation just like the other library nodes, so they
 * both a simulateIterationDeletion and simulateExecutionDeletion methods, which both obey to the same specifications
 * than {@link NodeDeletionHandler#simulateDeletion(List)}. However, while Iterations are candidates to mass-deletion, 
 * Executions are always deleted one at a time. That's why {@link #deleteExecution(Execution)} have a different contract
 * to enforce this.
 * 
 * On the other hand, <b>in the context of a cascade deletion</b> both Iterations and Executions will be removed <i>en masse</i>.  
 * 
 * Further remarks :
 * 
 ...
 *
 */
public interface CampaignNodeDeletionHandler extends
		NodeDeletionHandler<CampaignLibraryNode, CampaignFolder> {
	 ...
	 * the implementation should filter out the ids that can't be deleted.
	 * 
	 * 
	 * @param targetIds
