...
<s:url var="baseReportUrl"
	value="/report-workspace/report/generate?report=${report.id}">
</s:url>

<s:url var="infoReportUrl"
	value="/report-workspace/report/info?report=${report.id}">
</s:url>

<s:url var="baseReportExportOptionUrl"
...
<c:if test="${! param.hasBackButton}">
	<f:message var="toggleStateButtonNormal"
		key="report.workspace.togglebutton.normal.label" />
	<f:message var="toggleStateButtonExpand"
		key="report.workspace.togglebutton.expand.label" />
	...
	<c:if test="${! param.hasBackButton}">
		...
		<div id="category-frame-button">
			<input type="button" class="button"
				id="toggle-expand-category-frame-button"
				value="${toggleStateButtonNormal}" />
		</div>
	</c:if>

	<div style="float: left; height: 100%;">
		<h2>
			<span><f:message key="report.header.title" />&nbsp;:&nbsp;</span><a
				href="${infoReportUrl}">${reportName} (${reportType})</a>
		</h2>
	</div>
...
<div class="fragment-body">
	...
	<comp:toggle-panel id="report-criteria-panel" isContextual="true"
		...
		<%-- 
		 ...
		 <c:choose>
		 <c:when
					test="${report.class.simpleName=='ReportExecutionProgressFollowUp'}">
			<report:execution-progress-followup-criteria />
		</c:when>
		<c:when
					test="${report.class.simpleName=='ReportRequirementCoverageByTests'}">
			<report:requirement-coverage-criteria />
		</c:when>
		</c:choose>
			
		
			<f:message var="generateButtonLabel"
				key="report.criteria.panel.button.generate.label" />
			<input type="button" class="button" style="float: right;"
				id="generate" value="${generateButtonLabel}" />
			<div style="clear: both;"></div>
		</jsp:attribute>
	...
	<div id="view-tabed-panel" class="not-displayed">
		<ul>
			...
			<div id="export-option-div" style="float: right;">
				<span><f:message key="report.view.panel.label.export.label" />
				</span> <select id="export-select">
				</select>

				<f:message var="exportButtonLabel"
					key="report.view.panel.button.export.label" />
				<input type="button" class="button" id="export"
