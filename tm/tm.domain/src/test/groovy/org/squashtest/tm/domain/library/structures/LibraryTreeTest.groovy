...
class LibraryTreeTest extends Specification{
	...
	def "should sort a list of data (see comments of LibraryTree#sortData)"(){
		...
		given : 
			...
			def unsortedData = tree.toListOfPair([ [12, 23], [11, 22] , [1, 12], [11, 21], [null, 1], [1, 11] ])
		
		
		when :
			def test = tree.sortData(unsortedData)
		
		
		then :
			/*def fTest = test.flatten();
			def fsorted = sortedData.flatten();*/
		
			test.size() == 6	
		
			for (int i=0;i<6;i++){
				test[i].equals(sortedData[i])
			}
