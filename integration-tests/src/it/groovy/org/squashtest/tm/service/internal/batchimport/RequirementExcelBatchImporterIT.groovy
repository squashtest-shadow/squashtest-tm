...
class RequirementExcelBatchImporterIT extends DbunitServiceSpecification{
	...
	def setup(){
		...
		def attachVersionToReq = {
			reqId, vId ->Requirement req = findReq(reqId); req.addVersion(findVersion(vId))
		}

		def reqVersionMap = [11:[11, 12, 13], 21:[21], 31:[31], 41:[41]]
		reqVersionMap.each{
			reqId, versionIds -> versionIds.each{
	...
	def "should show errors during coverage import"(){
		...
		def errors = [Messages.ERROR_FIELD_MANDATORY, 
